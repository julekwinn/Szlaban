...
#define RTC_ALARM_TICK_PER_MS                       (2.048/1000)           // 1/2.048 = tick duration in ms
...
// Zakomentowane, bo nie zamierzamy wchodzić w tryb uśpienia (przynajmniej na początku) - PK
...
RtcCalendar_t RtcConvertTimerTimeToCalendarTick( TimerTime_t timeCounter )
{
    ...
    timeCounterTemp = ( double )timeCounter * RTC_ALARM_TICK_PER_MS; // / 1000.0;    // "/1000.0" bugfix by PK
...
void RtcGetTimeStr(uint8_t* buf)
{
	RtcCalendar_t calendar;
//	calendar = RtcGetCalendar();

	TimerTime_t tt = RtcGetTimerValue();
	calendar = RtcConvertTimerTimeToCalendarTick(tt);

	sprintf((char*)buf, "%02u:%02u:%02u",  calendar.CalendarTime.Hours, calendar.CalendarTime.Minutes, calendar.CalendarTime.Seconds);
}
